...
final class LibSkin {
	...
	private const SKIN_WIDTH_MAP = [
		64 * 32 * 4 => 64,
		64 * 64 * 4 => 64,
		128 * 128 * 4 => 128
	];

	private const SKIN_HEIGHT_MAP = [
		64 * 32 * 4 => 32,
		64 * 64 * 4 => 64,
		128 * 128 * 4 => 128
	];
	...
	public static function skinDataToImage(string $skinData) {
		...
		for ($y = 0; $y < $height; $y++) {
			for ($x = 0; $x < $width; $x++) {
				$r = ord($skinData[$skinPos]); $skinPos++;
				$g = ord($skinData[$skinPos]); $skinPos++;
				$b = ord($skinData[$skinPos]); $skinPos++;
				$a = 127 - intdiv(ord($skinData[$skinPos]), 2); $skinPos++;
